...
public class Application {
	...
	public void sortVector(Vector<Request> vecReqTo){
		Request sorted;
		if (vecReqTo.size() != 0)
	...
	public void printResult(PrintWriter print, PrintWriter print2, Vector<Request> vecReqTo, Vector<Status> vecStatus){
	...
	public void readLine(String text, Vector readCharRq, Vector readCharSt){
		
		Boolean boolrq = false;
	...
	public void concatRq(Request reqTo, Vector readCharRq){
	...
	public void concatSt(Status status, Vector readCharSt){
	...
	public void finalVecRq(Request reqTo, Vector<Request> vecReqTo){
		
		boolean findrq = false;
	...
	public void finalVecSt(Status status, Vector<Status> vecStatus){
	...
	public void readArq(String text, BufferedReader reader,Vector readCharRq, Vector readCharSt, Vector<Request> vecReqTo, Vector<Status> vecStatus) throws IOException{
		
		//Comando de repetição para ler o aquivo de entrada. Realizará a leitura enquanto não chegar ao final do mesmo
	    while ((text = reader.readLine()) != null) {
	    	
	    	//Comando de repetição para percorer a linha lida do arquivo de entrada
	    	readLine(text, readCharRq, readCharSt);
	    		    	
	    	//Inicializa um Objeto do tipo Request
	    	Request reqTo = new Request();
	    	reqTo.request_to = "";
	    	reqTo.cont = 1;
	    	...
	    	//Concatena caracteres formando a String final.
	    	concatRq(reqTo, readCharRq);
	    	concatSt(status, readCharSt);
			
			//Limpa os vetores para receber os próximos resultados lidos. 
			readCharRq.removeAllElements();
			readCharSt.removeAllElements();
			
			//Gera os vetores finais de request_to e response_status
			finalVecRq(reqTo, vecReqTo);
			finalVecSt(status, vecStatus);
	...
	public static void main(String[] args) {
		...
		try {
		    ...
		    //Trata os dados do arquivo de entrada e separa-os em vetores finais com os objetos de Request e Status
		    app.readArq(text, reader, readCharRq, readCharSt, vecReqTo, vecStatus);
		    
		    //Ordenação do Vetor final de objetos Request
		    app.sortVector(vecReqTo);
		    
		    //Impressão de resultados em arquivos de saída e no console da IDE
		    app.printResult(print, print2, vecReqTo, vecStatus);
